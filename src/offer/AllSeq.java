...
class AllSeqHaveSame {
    //没有考虑输入为空串的情况
    public ArrayList<String> Permutation(String str) {
        char[] array = str.toCharArray();
        Arrays.sort(array);
        Set<String> set = new HashSet<>();
        AllSeqSort(set, array, 0);
        ArrayList<String> result = new ArrayList<>(set);
        Collections.sort(result);
        return result;
    }

    public void AllSeqSort(Set<String> set, char[] array, int index) {
        if (index == array.length) {
            set.add(new String(array));
        }
        for (int i = index; i < array.length; i++) {
            sweap(array, index, i);
            AllSeqSort(set, array, index + 1);
            //todo 不交换回来会有问题。
            sweap(array, index, i);
        }
    }

    private void sweap(char[] array, int a, int b) {
        if (a != b) {
            char temp = array[a];
            array[a] = array[b];
            array[b] = temp;
    ...
    public static void main(String[] args) {
        AllSeqHaveSame test = new AllSeqHaveSame();
